...
echo "
		 ...
		 while($rows = mysqli_fetch_assoc($run) ) {
		 	echo "
	 			...
	 			<th><a href='edit_uni_courses.php?edit_id=$rows[course_id]' class='btn btn-primary'>Edit</button></a></th>
	 			<th><a href='edit_uni_courses.php?del_id=$rows[course_id]' class='btn btn-danger'>Delete</button></a></th>
		 		</tr>
		 	";
		 $num++;
		 }
			...
			<form method="POST" >
				...
				<div class="col-md-5">
					...
					<label>Requirements:</label>
						<textarea class="form-control" name="requirements" value="<?php echo $requirements;?>"></textarea>
					</div>
					<center>
						...
						<a href="edit_uni_courses.php" class="btn btn-danger">Cancel</a>
					</center>
				</div>
	</form>
	</center>
...
if (isset($_POST['sub_courses'] ) ){
	...
	$edit_sql = "UPDATE all_course_details SET course_name= '$course_name', depart = '$depart', category = '$category', faculty='$faculty', duration = '$duration', institution= '$institution', location = '$location', intake = '$intake', exam_body = '$exam_body', pricing='$pricing', requirements = '$requirements' WHERE course_id ='$edit_id' ";
	 ...
	 if(mysqli_query($conn,$edit_sql) ) { ?>
	 	...
	 	window.location="edit_uni_courses.php";
	 	</script>
	 <?php } else { ?>
	 <script >
	 window.alert("Server Error. Retry");
